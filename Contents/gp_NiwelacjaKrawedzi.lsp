...
(defun get-safe-proj-2d (crv pt / p-proj)
  (setq p-proj (vl-catch-all-apply 'vlax-curve-getClosestPointToProjection (list crv pt '(0.0 0.0 1.0))))
          ...
          (progn   
            ...
            (if projPt   
              (progn   
                ...
                (if (< (distance (list (car pt) (cadr pt)) (list (car projPt) (cadr projPt))) 0.05)   
  ...
  (if (null ptsList)    
    (progn (alert "Nie znaleziono pikiet lezacych na linii zrodlowej!") (exit))   
  )   
      ...
      (progn   
        ...
        (if (> dist2d 0.001)  
          (progn  
            ...
            (setq safe-pt-tgt (vlax-curve-getClosestPointTo tgtObj pt-tgt))
            (setq param-catch (vl-catch-all-apply 'vlax-curve-getParamAtPoint (list tgtObj safe-pt-tgt)))
            ...
            (if (not (vl-catch-all-error-p param-catch))
              (progn
                ...
                ;; Tolerancja prostopadłości poluzowana do 0.15 (ok. 8,6 stopnia). Skrypt nie urywa się na drobnych krzywiznach.
                (if (> cos-theta 0.15)  
